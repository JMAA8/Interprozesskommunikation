...
int main(int argc, char *argv[])
{
  ...
  // P-Operation definieren. Wert der Semaphore um eins dekrementieren
  struct sembuf r_operation = {0, -1, 0};

  // V-Operation definieren. Wert der Semaphore um eins inkrementieren
  struct sembuf s_operation = {0, 1, 0};
  ...
  // P-Operation definieren. Wert der Semaphore um eins dekrementieren
  struct sembuf m_operation = {0, -1, 0};

  // V-Operation definieren. Wert der Semaphore um eins inkrementieren
  struct sembuf l_operation = {0, 1, 0};
  ...
  if (id1 == 0)
  {
    ...
    if (id2 == 0)
    {
      ...
      while (1)
      {
        ...
        int SemaphoreValue;
        SemaphoreValue = semctl(StatReport_semget2, 0, GETVAL, 0);
        printf("SemaphoreValue StatReport2 in Conv: %d\n", SemaphoreValue);

        semop(ConvLog_semget1, &p_operation, 1);

        printf("Semaphore 1 geprüft\n");
  ...
  else
  {
    if (id2 == 0)
    {
      ...
      while (1)
      {
        ...
        int SemaphoreValue;
        SemaphoreValue = semctl(StatReport_semget2, 0, GETVAL, 0);
        printf("SemaphoreValue StatReport2 in Stat vor inkrementieren: %d\n", SemaphoreValue);

        semop(ConvStat_semget1, &v_operation, 1);
        semop(StatReport_semget2, &v_operation, 1);

        int SemaphoreValue2;
        SemaphoreValue2 = semctl(StatReport_semget2, 0, GETVAL, 0);
        printf("SemaphoreValue StatReport2 in Stat nach inkrementieren: %d\n", SemaphoreValue2);
    ...
    else
    {
      ...
      while (1)
      {
        ...
        int SemaphoreValue;
        SemaphoreValue = semctl(StatReport_semget2, 0, GETVAL, 0);
        printf("SemaphoreValue StatReport2 in Report: %d\n", SemaphoreValue);

        if (printf("Summe Neu: %s\n", StatReportSpointer) < 0)
        {
          printf("Der Lesezugriff ist fehlgeschlagen.\n");
          exit(1);
        }

        if (printf("Mittelwert Neu: %s\n", StatReportMpointer) < 0)
        {
          printf("Der Lesezugriff ist fehlgeschlagen.\n");
          exit(1);
        }
